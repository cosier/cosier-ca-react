 ...
 *  i.e. one of: #nnn, #nnnnnn, rgb(), rgba(), hsl(), hsla()
 * @return {{type: string, values: number[]}} A MUI color object
 */
 ...
 *  i.e. one of: #nnn, #nnnnnn, rgb(), rgba(), hsl(), hsla()
 *
 * @return {number} A contrast ratio value in the range 0 - 21
 ...
 * @param {string} color - CSS color,
 *  i.e. one of: #nnn, #nnnnnn, rgb(), rgba(), hsl(), hsla()
 * @return {number} The relative brightness of the color in the range 0 - 1
 */
export function getLuminance(color) {
  ...
  if (color.type.indexOf('rgb') > -1) {
    const rgb = color.values.map((val) => {
      val /= 255; // normalized
      return val <= 0.03928 ?
        val / 12.92 : Math.pow((val + 0.055) / 1.055, 2.4);
    });
 ...
 * @return {string} A CSS color string. Hex input values are returned as rgb
 */
